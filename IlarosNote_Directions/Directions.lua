---------------------------------------------------------
-- Addon declaration
IlarosNote_Directions = LibStub("AceAddon-3.0"):NewAddon("IlarosNote_Directions","AceEvent-3.0","AceHook-3.0")
local HD = IlarosNote_Directions
local IlarosNote = LibStub("AceAddon-3.0"):GetAddon("IlarosNote")
local Astrolabe = DongleStub("Astrolabe-1.0")
local L = LibStub("AceLocale-3.0"):GetLocale("IlarosNote_Directions", true)
...
local function generateMenu(button, level)
	...
	if (level == 1) then
		...
		info.isTitle      = 1
		info.text         = "IlarosNote - Directions"
		info.notCheckable = 1
...
local HD_Dropdown = CreateFrame("Frame", "IlarosNote_DirectionsDropdownMenu")
...
local options = {
	...
	args = {
		desc = {
			name = "These settings control the look and feel of the icon. Note that IlarosNote_Directions does not come with any precompiled data, when you ask a guard for directions, it will automatically add the data into your database.",
			type = "description",
			order = 0,
		},
		icon_scale = {
...
function HD:OnInitialize()
	-- Set up our database
	self.db = LibStub("AceDB-3.0"):New("IlarosNote_DirectionsDB", defaults)
	db = self.db.profile
