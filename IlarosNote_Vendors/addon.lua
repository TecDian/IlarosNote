...
---------------------------------------------------------
-- Addon declaration
IlarosNote_Vendors = LibStub("AceAddon-3.0"):NewAddon("IlarosNote_Vendors","AceEvent-3.0")
local HV = IlarosNote_Vendors
local Astrolabe = DongleStub("Astrolabe-0.4")
local L = LibStub("AceLocale-3.0"):GetLocale("IlarosNote_Vendors")
...
local function deletePin(button, mapFile, coord)
	...
	HV:SendMessage("IlarosNote_NotifyUpdate", "Vendors")
end
...
local function generateMenu(button, level)
	...
	if (level == 1) then
		...
		info.isTitle      = 1
		info.text         = L["IlarosNote - Vendors"]
		info.notCheckable = 1
...
local HV_Dropdown = CreateFrame("Frame", "IlarosNote_VendorsDropdownMenu")
...
function HV:OnInitialize()
	-- Set up our database
	db = LibStub("AceDB-3.0"):New("IlarosNote_VendorsDB", defaults)
	self.db = db

	if db.factionrealm.dbversion > CURRENT_DB_VERSION then
		print("|cff6fafffIlarosNote_Vendors:|r |cffff4f00Warning:|r Unknown database version. Please update to newer version.")
		print("|cff6fafffIlarosNote_Vendors:|r |cffff4f00Warning:|r Addon has been disabled to protect your database.")
		self:Disable()
		return
	end
	...
	IlarosNote:RegisterPluginDB("Vendors", HVHandler, options)
end
...
--					return
		...
		self:SendMessage("IlarosNote_NotifyUpdate", "Vendors")
	end
end
