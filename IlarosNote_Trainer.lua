...
-- Modul-Deklaration
IlarosNote_Trainer = LibStub("AceAddon-3.0"):NewAddon("IlarosNote_Trainer","AceEvent-3.0")
local HT = IlarosNote_Trainer
...
setmetatable(tsNameDB, {__index = function (t, k)
                    local v = k
                    print("IlarosNote_Trainer: Untranslated tradeskill name found! Please report to the addon author.")
...
local HT_Dropdown = CreateFrame("Frame", "IlarosNote_TrainerDropdownMenu")
...
function HT:OnInitialize()
    -- Set up our database
    db = LibStub("AceDB-3.0"):New("NoteTrainers", defaults)
    self.db = db

    if db.factionrealm.dbversion > CURRENT_DB_VERSION then
        print("|cff6fafffIlarosNote_Trainer:|r |cffff4f00Warning:|r Unknown database version. Please update to newer version.")
        print("|cff6fafffIlarosNote_Trainer:|r |cffff4f00Warning:|r Addon has been disabled to protect your database.")
        self:Disable()
        return
    end
