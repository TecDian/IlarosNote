----------------------------------------------------------------------------
-- IlarosNote
-- Modul für Auktionatoren, Bankiers
-- Kampfmeister und Stallmeister funktionieren noch nicht
-- Barbier und Gildenbank funktionieren noch nicht, Optionen fehlen
-- Briefkasten in eigenem Modul - hier integrieren
-- Geistheiler in eigenem Modul - Option entfernen
----------------------------------------------------------------------------

-- Modul-Deklaration
IlarosNote_City = LibStub("AceAddon-3.0"):NewAddon("IlarosNote_City","AceEvent-3.0")
local HC = IlarosNote_City
...
local function deletePin(button, mapFile, coord)
    ...
    HC:SendMessage("IlarosNote_NotifyUpdate", "CityGuide")
end
...
local function generateMenu(button, level)
    ...
    if (level == 1) then
        ...
        info.isTitle      = 1
        info.text         = L["IlarosNote - CityGuide"]
        info.notCheckable = 1
...
local HC_Dropdown = CreateFrame("Frame", "IlarosNote_CityDropdownMenu")
...
function HC:OnInitialize()
    -- Set up our database
    db = LibStub("AceDB-3.0"):New("NoteCities", defaults)
    self.db = db

    if db.factionrealm.dbversion > CURRENT_DB_VERSION then
        print("|cff6fafffIlarosNote_City:|r |cffff4f00Warning:|r Unknown database version. Please update to newer version.")
        print("|cff6fafffIlarosNote_City:|r |cffff4f00Warning:|r Addon has been disabled to protect your database.")
        self:Disable()
        return
    end
...
db.factionrealm.dbversion = CURRENT_DB_VERSION
    ...
    IlarosNote:RegisterPluginDB("CityGuide", HCHandler, options)
end
...
--                  coord = coords  -- let's not move, just update vInfo
        ...
        self:SendMessage("IlarosNote_NotifyUpdate", "CityGuide")
    end
end
