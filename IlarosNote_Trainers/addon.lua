...
---------------------------------------------------------
-- Addon declaration
IlarosNote_Trainers = LibStub("AceAddon-3.0"):NewAddon("IlarosNote_Trainers","AceEvent-3.0")
local HT = IlarosNote_Trainers
local Astrolabe = DongleStub("Astrolabe-0.4")
local L = LibStub("AceLocale-3.0"):GetLocale("IlarosNote_Trainers")
...
---------------------------------------------------------
-- Constants
local iconpath = "Interface\\AddOns\\IlarosNote_Trainers\\Artwork\\"
...
setmetatable(tsNameDB, {__index = function (t, k)
					local v = k
					print("IlarosNote_Trainers: Untranslated tradeskill name found! Please report to the addon author.")
...
local function deletePin(button, mapFile, coord)
	...
	HT:SendMessage("IlarosNote_NotifyUpdate", "Trainers")
end
...
local function generateMenu(button, level)
	...
	if (level == 1) then
		...
		info.isTitle      = 1
		info.text         = L["IlarosNote - Trainers"]
		info.notCheckable = 1
...
local HT_Dropdown = CreateFrame("Frame", "IlarosNote_TrainersDropdownMenu")
...
function HT:OnInitialize()
	-- Set up our database
	db = LibStub("AceDB-3.0"):New("IlarosNote_TrainersDB", defaults)
	self.db = db

	if db.factionrealm.dbversion > CURRENT_DB_VERSION then
		print("|cff6fafffIlarosNote_Trainers:|r |cffff4f00Warning:|r Unknown database version. Please update to newer version.")
		print("|cff6fafffIlarosNote_Trainers:|r |cffff4f00Warning:|r Addon has been disabled to protect your database.")
		self:Disable()
		return
	end
	...
	IlarosNote:RegisterPluginDB("Trainers", HTHandler, options)
end
...
--					return
		...
		self:SendMessage("IlarosNote_NotifyUpdate", "Trainers")
	end
end
