...
---------------------------------------------------------
-- Addon declaration
IlarosNote_Charon = LibStub("AceAddon-3.0"):NewAddon("IlarosNote_Charon","AceEvent-3.0", "AceTimer-3.0")
local HC = IlarosNote_Charon
local Astrolabe = DongleStub("Astrolabe-0.4")
local L = LibStub("AceLocale-3.0"):GetLocale("IlarosNote_Charon")
local G = {}
...
---------------------------------------------------------
-- Constants
local iconpath = "Interface\\AddOns\\IlarosNote_Charon\\Artwork\\"
...
local HC_Dropdown = CreateFrame("Frame", "IlarosNote_CharonDropdownMenu")
...
function HC:OnInitialize()
	-- Set up our database
	db = LibStub("AceDB-3.0"):New("IlarosNote_CharonDB", defaults)
	self.db = db
	self.data = CharonData

	if not IlarosNote_CharonCharDB then
		IlarosNote_CharonCharDB = {
			dbversion = CURRENT_DB_VERSION,
			nodes = {}
		}
	end
	CharDB = IlarosNote_CharonCharDB


	if (db.factionrealm.dbversion > CURRENT_DB_VERSION) or (CharDB.dbversion > CURRENT_DB_VERSION) then
		print("|cff6fafffIlarosNote_Charon:|r |cffff4f00Warning:|r Unknown database version. Please update to newer version.")
		print("|cff6fafffIlarosNote_Charon:|r |cffff4f00Warning:|r Addon has been disabled to protect your database.")
		self:Disable()
		return
	end
...
function G:DrawLine(C, sx, sy, ex, ey, w, color, layer)
	...
	T:SetTexture("Interface\\AddOns\\IlarosNote_Charon\\line")
