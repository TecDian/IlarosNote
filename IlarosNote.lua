...
---------------------------------------------------------
-- Addon declaration
IlarosNote = LibStub("AceAddon-3.0"):NewAddon("IlarosNote", "AceConsole-3.0", "AceEvent-3.0")
...
local function getNewPin()
	...
	-- create a new pin
	pinCount = pinCount + 1
	pin = CreateFrame("Button", "IlarosNotePin"..pinCount, WorldMapButton)
	pin:SetFrameLevel(5)
...
--[[ Documentation:
IlarosNote.plugins table contains every plugin which we will use to iterate over.
...
Standard functions you can provide optionally:
	pluginHandler:OnEnter(mapFile, coord)
		Function we will call when the mouse enters a IlarosNote, you will generally produce a tooltip here.
	pluginHandler:OnLeave(mapFile, coord)
		Function we will call when the mouse leaves a IlarosNote, you will generally hide the tooltip here.
	pluginHandler:OnClick(button, down, mapFile, coord)
		Function we will call when the user clicks on a IlarosNote, you will generally produce a menu here on right-click.
]]

function IlarosNote:RegisterPluginDB(pluginName, pluginHandler, optionsTable)
...
function pinsHandler:OnEnter(motion)
	...
	safecall(IlarosNote.plugins[self.pluginName].OnEnter, self, self.mapFile, self.coord)
end
function pinsHandler:OnLeave(motion)
	WorldMapBlobFrame:SetScript("OnUpdate", WorldMapBlobFrame_OnUpdate) -- restore default UI
	safecall(IlarosNote.plugins[self.pluginName].OnLeave, self, self.mapFile, self.coord)
end
function pinsHandler:OnClick(button, down)
	safecall(IlarosNote.plugins[self.pluginName].OnClick, self, button, down, self.mapFile, self.coord)
end
...
function IlarosNote:GetMapFile(C, Z)
...
function IlarosNote:GetZoneToCZ(zone)
	return reverseZoneC[zone], reverseZoneZ[zone]
end
function IlarosNote:GetCZToZone(C, Z)
	if not C or not Z then return end
	if Z == 0 then
...
function IlarosNote:UpdateWorldMapPlugin(pluginName)
...
function IlarosNote:UpdateWorldMap()
	if not WorldMapButton:IsVisible() then return end
...
-- This function updates all the icons of one plugin on the world map
function IlarosNote:UpdateMinimapPlugin(pluginName)
	...
	local continent, zone = IlarosNote:GetZoneToCZ(GetRealZoneText())
...
function IlarosNote:UpdateMinimap()
	--if not Minimap:IsVisible() then return end
	for pluginName, pluginHandler in pairs(self.plugins) do
...
-- This function runs when we receive a "IlarosNote_NotifyUpdate"
-- notification from a plugin that its icons needs to be updated
-- Syntax is plugin:SendMessage("IlarosNote_NotifyUpdate", "pluginName")
function IlarosNote:UpdatePluginMap(message, pluginName)
...
function IlarosNote.AstrolabeEdgeCallback()
	for pluginName, pluginHandler in pairs(IlarosNote.plugins) do
		for coordID, icon in pairs(minimapPins[pluginName]) do
...
options = {
	...
	args = {
		enabled = {
			type = "toggle",
			name = L["Enable IlarosNote"],
			desc = L["Enable or disable IlarosNote"],
			order = 1,
			get = function(info) return db.enabled end,
			set = function(info, v)
				db.enabled = v
				if v then IlarosNote:Enable() else IlarosNote:Disable() end
			end,
			disabled = false,
		},
		overall_settings = {
			...
			args = {
				desc = {
					name = L["These settings control the look and feel of IlarosNote globally. The icon's scale and alpha here are multiplied with the plugin's scale and alpha."],
					type = "description",
					order = 0,
				},
				icon_scale = {
			...
			args = {
				...
				show_plugins = {
					...
					set = function(info, k, v)
						...
						IlarosNote:UpdatePluginMap(nil, k)
					end,
				},
			},
		},
...
function IlarosNote:OnInitialize()
	-- Set up our database
	self.db = LibStub("AceDB-3.0"):New("IlarosNoteDB", defaults)
	...
	LibStub("AceConfigRegistry-3.0"):RegisterOptionsTable("IlarosNote", options)
	self:RegisterChatCommand("IlarosNote", function() LibStub("AceConfigDialog-3.0"):Open("IlarosNote") end)
	LibStub("AceConfigDialog-3.0"):AddToBlizOptions("IlarosNote", "IlarosNote")
...
function IlarosNote:OnEnable()
	if not db.enabled then
		self:Disable()
		return
	end
	self:RegisterEvent("WORLD_MAP_UPDATE", "UpdateWorldMap")
	self:RegisterMessage("IlarosNote_NotifyUpdate", "UpdatePluginMap")
...
function IlarosNote:OnProfileChanged(event, database, newProfileKey)
	db = database.profile
	self:UpdateMinimap()
	self:UpdateWorldMap()
end
