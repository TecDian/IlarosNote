----------------------------------------------------------------------------
-- IlarosNote
-- Modul für Gildenmitglieder
----------------------------------------------------------------------------

-- Modul-Deklaration
local IlarosNote_Guild = LibStub("AceAddon-3.0"):NewAddon("IlarosNote_Guild", "AceEvent-3.0", "AceBucket-3.0", "AceHook-3.0")
...
local defaults = nil
local options = nil
local icon = "Interface\\AddOns\\IlarosNote\\images\\normal"
...
do
	...
	-- evil hack to not show northrend people outside of northrend zones
	local Astrolabe = DongleStub("Astrolabe-0.4")
	local northrendZones = {Northrend = true, World = true}
	for k,v in pairs(Astrolabe.ContinentList[4]) do
		northrendZones[v] = true
	end
	...
	local function iter(t)
		...
		prestate = state
		-- evil hack to not show northrend people outside of northrend zones
		if state and (northrendZones[map] == northrendZones[value.zone]) then
...
function IlarosNote_Guild:OnInitialize()
	-- Set up our database
	self.db = LibStub("AceDB-3.0"):New("NoteGuilds", defaults)
	
	-- Initialize our database with IlarosNote
	IlarosNote:RegisterPluginDB(L["Guildmembers"], GuildHandler, options)
end

function IlarosNote_Guild:OnEnable()
...
function IlarosNote_Guild:OnDisable()
...
function IlarosNote_Guild:GUILD_ROSTER_UPDATE()
	...
	if updateNeeded then
		self:SendMessage("IlarosNote_NotifyUpdate", "Guildmembers")
	end
end

-- LGP Callback for updating member positions
function IlarosNote_Guild:UpdateMember(event, sender, x, y, zone)
	if event == "Clear" then
		recycle(database[sender])
		database[sender] = nil
	elseif event == "Position" then
		database[sender].coord = IlarosNote:getCoord(x,y)
...
function IlarosNote_Guild:UpdateAllMembers()
	for name, x, y, zone in LGP:IterateGuildMembers() do
		database[name].coord = IlarosNote:getCoord(x,y)
