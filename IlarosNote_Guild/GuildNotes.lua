--[[ $Id: GuildNotes.lua 42 2008-12-04 08:47:47Z nevcairiel $ ]]
local Guild = LibStub("AceAddon-3.0"):NewAddon("IlarosNote_Guild", "AceEvent-3.0", "AceBucket-3.0", "AceHook-3.0")
...
local defaults = nil
local options = nil
local icon = "Interface\\AddOns\\IlarosNote_Guild\\Normal"
...
function Guild:OnInitialize()
	-- Set up our database
	self.db = LibStub("AceDB-3.0"):New("IlarosNote_GuildDB", defaults)
	
	-- Initialize our database with IlarosNote
	IlarosNote:RegisterPluginDB("Guild", GuildHandler, options)
end
...
function Guild:GUILD_ROSTER_UPDATE()
	...
	if updateNeeded then
		self:SendMessage("IlarosNote_NotifyUpdate", "Guild")
	end
end
