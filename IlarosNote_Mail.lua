----------------------------------------------------------------------------
-- IlarosNote
-- Modul für die Briefkästen
----------------------------------------------------------------------------

-- Modul-Deklaration
IlarosNote_Mail = LibStub("AceAddon-3.0"):NewAddon("IlarosNote_Mail","AceEvent-3.0")
local HMB = IlarosNote_Mail
...
---------------------------------------------------------
-- Constants
local icon = "Interface\\AddOns\\IlarosNote\\images\\mail.tga"
...
local function generateMenu(button, level)
    ...
    if (level == 1) then
        ...
        info.isTitle      = 1
        info.text         = L["IlarosNote - Mailbox"]
        info.notCheckable = 1
...
local HMB_Dropdown = CreateFrame("Frame", "IlarosNote_MailDropdownMenu")
...
local options = {
    ...
    args = {
        desc = {
            name = L["These settings control the look and feel of the Mailbox icon."],
            type = "description",
            order = 0,
        },
        icon_scale = {
...
function HMB:OnInitialize()
    -- Set up our database
    self.db = LibStub("AceDB-3.0"):New("NoteMails", defaults)
    db = self.db.profile
    self.usingSquire = false
